...
CRS_KOREA = "EPSG:5179"
GRID_SIZE = 100
VULN_DIST = 1000    # 교통 공백 기준: N버스 정류장 1km 초과
...
def visualize(grid, seoul, n_gdf):
    ...
    # N버스 정류장 (노란 점)
    ax.scatter([g.x for g in n_gdf.geometry], [g.y for g in n_gdf.geometry],
               c='#FFD700', s=4, alpha=0.8, zorder=5)
...
if __name__ == '__main__':
    ...
    grid, n_gdf     = map_nbus(grid)
    grid            = map_lights(grid)
    grid            = flag_combined(grid)
    visualize(grid, seoul, n_gdf)
    save_results(grid)
